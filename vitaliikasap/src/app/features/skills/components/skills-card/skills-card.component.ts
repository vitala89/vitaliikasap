...
@Component({
  ...
  template: `
    <div class="flex flex-col bg-white/90 dark:bg-neutral-800/90 rounded-3xl shadow-2xl p-8 max-h-[800px] overflow-y-auto scrollbar-hide max-w-5xl mx-auto">
      ...
      <div class="flex flex-wrap gap-6 mb-10">
        @for (lang of languageSkills(); track lang.name) {
          <div class="flex flex-col items-center w-32 hover:scale-110">
            ...
            <div class="text-xl font-semibold dark:text-neutral-200">{{ lang.value }}%</div>
            <div class="mt-2 text-base text-neutral-700 dark:text-neutral-200">{{ lang.name }}</div>
          </div>
        }
      </div>

      <!-- Hard Skills -->
      <h2 class="text-xl font-bold mb-3 dark:text-white">{{ t('skills.hard.title') }}</h2>
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-6">
        @for (hard of hardSkills(); track hard.name) {
          <div class="cursor-hover flex flex-col items-center justify-center py-6 rounded-3xl bg-neutral-50 dark:bg-neutral-900 shadow-sm border border-neutral-100 dark:border-neutral-700 transition hover:shadow-lg hover:shadow-gray-400">
      ...
      <!-- Soft Skills -->
      <h2 class="text-xl font-bold mt-8 mb-3 dark:text-white">{{ t('skills.soft.title') }}</h2>
      <ul class="flex flex-wrap gap-4">
        @for (soft of softSkills(); track soft) {
          <li class="cursor-hover px-4 py-2 rounded-2xl bg-indigo-100 dark:bg-indigo-800/40 text-indigo-600 dark:text-indigo-300 font-semibold hover:shadow-lg hover:shadow-gray-400">{{ soft }}</li>
        }
      </ul>
    </div>
  `
